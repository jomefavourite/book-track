...
1. **Env checklist**
   - Convex: `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY` (dashboard → Settings → Environment Variables).
...
2. **Push**
   - The service worker is disabled in `npm run dev`, so **push only works when running a production build**: `npm run build && npm start` (or deploy). Then enable reminders in **Dashboard → Settings**, set a time and timezone, then **Save settings** (grant notification permission when prompted).
